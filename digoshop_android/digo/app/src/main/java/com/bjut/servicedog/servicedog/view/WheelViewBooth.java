...
public class WheelViewBooth extends ScrollView {
...
//    private ScrollView scrollView;
    ...
    public WheelViewBooth(Context context) {
        super(context);
        init(context);
    }

    public WheelViewBooth(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }

    public WheelViewBooth(Context context, AttributeSet attrs, int defStyle) {
    ...
    private List<Booth> getItems() {
        return items;
    }

    public void setItems(List<Booth> list) {
        if (null == items) {
            items = new ArrayList<>();
        }
        items.clear();
...
//                    Log.d(TAG, "remainder: " + remainder + ", divided: " + divided);
                    if (remainder == 0) {
                        ...
                        if (remainder > itemHeight / 2) {
                            WheelViewBooth.this.post(new Runnable() {
                                @Override
                                public void run() {
                                    WheelViewBooth.this.smoothScrollTo(0, initialY - remainder + itemHeight);
                            ...
                            WheelViewBooth.this.post(new Runnable() {
                                @Override
                                public void run() {
                                    WheelViewBooth.this.smoothScrollTo(0, initialY - remainder);
                                    selectedIndex = divided + offset;
                                    onSeletedCallBack();
                                }
                            });
                    ...
                    initialY = getScrollY();
                    WheelViewBooth.this.postDelayed(scrollerTask, newCheck);
                }
            }
        };
    ...
    private void initData() {
        ...
        for (Booth item : items) {
            views.addView(createView(item.getTag()));
        }
...
//            position = divided + offset + 1;
    ...
    public void setSeletion(int position) {
        ...
        this.post(new Runnable() {
            @Override
            public void run() {
                WheelViewBooth.this.smoothScrollTo(0, p * itemHeight);
            }
        });

    }

    public Booth getSeletedItem() {
