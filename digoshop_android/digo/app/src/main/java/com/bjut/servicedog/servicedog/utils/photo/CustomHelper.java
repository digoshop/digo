package com.bjut.servicedog.servicedog.utils.photo;
...
public class CustomHelper {
    ...
    private boolean isCrop = true;//是否剪裁
    ...
    int width = 800;//宽
    int height = 800;//高
    private int etCropWidth = 1;
    private int etCropHeight = 1;
    ...
    public void onClick(boolean isChoise, TakePhoto takePhoto) {
        String path = "";
        String state = Environment.getExternalStorageState();
        if (state.equals(Environment.MEDIA_MOUNTED)) {
            path = Environment.getExternalStorageDirectory().getAbsolutePath();
        } else {
            ToastUtils.showShortToastSafe("请检查SDCard设置");
            return;
        }
