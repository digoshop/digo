...
import com.bjut.servicedog.servicedog.adapter.CategoryWheelAdapter;
...
public class ChooseCategoryWheel implements MyOnWheelChangedListener {
    ...
    private List<RunKind> runKind = null;
    ...
    public ChooseCategoryWheel(Activity context) {
    ...
    private void bindData() {
        CategoryWheelAdapter categoryWheelAdapter = new CategoryWheelAdapter(context, runKind);
        categoryWheelAdapter.setTextSize(base_dimen_40);
        categoryWheel.setViewAdapter(categoryWheelAdapter);
    }
    ...
    public void setProvince(List<RunKind> runKind) {
    ...
    public void confirm() {
        if (onCategortChangeListener != null) {
            ...
            if (runKind != null && runKind.size() > floorIndex) {
                // AddressDtailsEntity.ProvinceEntity provinceEntity = province.get(provinceIndex);
                floorName = runKind.get(floorIndex).getName();
                moid = runKind.get(floorIndex).getMoid();
            }
            onCategortChangeListener.onFoorChange(floorName, moid);
