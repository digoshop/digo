...
public class UpdateProductActivity extends BaseActivity implements TakePhoto.TakeResultListener, InvokeListener {
...
//    private EditText et_notice;
    ...
    private void setView() {
        operateId = mProduct.getMoid();
        if (StringUtils.isEmpty(mProduct.getBid())) {
            brandId = "";
        } else {
            brandId = mProduct.getBid();
        }
        et_name.setText(mProduct.getPna());
        et_no.setText(mProduct.getPno());
        tv_type.setText(mProduct.getMon());
        tv_brand.setText(mProduct.getBn());
        et_market_price.setText(mProduct.getPpr());
        et_favorable_price.setText(mProduct.getPppr());
        et_stock.setText(mProduct.getPnu() + "");
        et_property.setText(mProduct.getPattr());
        et_introduce.setText(mProduct.getPd());
//        et_notice.setText(mProduct.getPasa());
        for (String s : mProduct.getPa()) {
...
//        et_notice = (EditText) findViewById(et_notice);
        ...
        pafr = getIntent().getStringExtra("pafr");
...
//        String notice = et_notice.getText().toString();
        ...
        if (!StringUtils.isEmpty(marketPrice) && !StringUtils.isEmpty(favorablePrice)) {
            int market = Integer.parseInt(marketPrice);
            int favorable = Integer.parseInt(favorablePrice);
            if (market>favorable) {
                toast("现价不能大于原价");
                return false;
            }
...
//        }
    ...
    private void sendRequese() {
        ...
        urlString = String.format(urlString);
        Map<String, String> map = new HashMap<>();
        map.put("productId", mProduct.getPid() + "");
                    ...
                    public void run() {
                        ...
                        try {
                            ...
                            if (jsonObject.getJSONObject("e").getInt("code") == 0) {
                                ...
                                toast("商品已成功提交，系统会在24小时内进行审核，请耐心等候，介时查阅！");
