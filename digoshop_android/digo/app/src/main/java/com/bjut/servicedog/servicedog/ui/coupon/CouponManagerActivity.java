...
import com.bjut.servicedog.servicedog.rc_adapter.CouponAdapter;
...
public class CouponManagerActivity extends BaseActivity implements View.OnClickListener, SwipeRefreshLayout.OnRefreshListener, BaseQuickAdapter.RequestLoadMoreListener {

    private int[] status = {R.id.left, R.id.middle, R.id.right};
    private int[] kind = {R.id.kindall, R.id.kind1, R.id.kind2, R.id.kind3};
    //    private int[] backgroud = {R.drawable.fabuleft, R.drawable.fabumiddle, R.drawable.faburight};
    ...
    private String auditType;//1未审核通过  2通过
    private Intent auditIntent;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        init();
        setListener();
        requestCouponList(page, statu, type);
    }
        ...
        mRecyclerView.addOnItemTouchListener(new OnItemClickListener() {
            ...
            public void onSimpleItemClick(BaseQuickAdapter adapter, View view, int position) {
                if (Utils.isNotFastClick()) {
                    ...
                    auditIntent = new Intent(CouponManagerActivity.this, CheckCouponActivity.class);
                    String rule = String.valueOf(item.getCouponBatch().getRule());
                    int status = item.getCouponBatch().getStatus();
                    ...
                    if (1 == item.getCouponBatch().getCbas() || 2 == item.getCouponBatch().getCbas()) {
                        auditType = "1";
                        auditIntent.putExtra("auditType", auditType);
                        auditIntent.putExtra("rule", rule);
                        auditIntent.putExtra("status", status);
    ...
    public void init() {
        ...
        mAdapter = new CouponAdapter(mData);
...
//        mAdapter.openLoadAnimation(Constant.LOAD_ANIMATION);
    ...
    public void requestCouponList(final int page, int Status, int Type) {
        if (loadType == 0) {
            showProgressDialog();
        }
        if (Type == 0)
...
//                mData.clear();
                ...
                break;
            case R.id.btn_right:
                myIntentR(AddDiscountCouponManageActivity_f.class);
                break;
        }
    }
